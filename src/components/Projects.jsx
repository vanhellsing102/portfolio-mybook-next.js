...
const Projects = () => {
    return (
        <div id="projects" className='md:mx-14 mx-0'>
                    ...
                    projects.map(project =>
                        <Tilt key={project?.id}
                            ...
                            <div className="inner-element flex flex-col h-full justify-between">
                              ...
                              <h2 className='text-2xl font-medium text-[#F8F8F8] capitalize mt-2'>{project?.projectName}</h2>
                              <p className='text-[#A3B3C2] text-sm'>{project?.details}</p>
                              <p className='flex justify-center gap-2 items-center mt-2'>{project?.features.map(item =>
